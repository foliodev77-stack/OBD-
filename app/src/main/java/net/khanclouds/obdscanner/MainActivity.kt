...
class MainActivity:AppCompatActivity(){
 ...
 override fun onCreate(b:Bundle?){super.onCreate(b);setContentView(R.layout.activity_main)
  ...
  findViewById<Button>(R.id.connectBtn).setOnClickListener{refresh();if(devices.isEmpty()){status.text="No paired OBD adapter found";return@setOnClickListener};status.text="Connecting to adapter..."
   thread{try{client.connect(devices[spinner.selectedItemPosition.coerceAtLeast(0)]);runOnUiThread{status.text="Connecté ✓ • Étape 2 : lancez VIN Scan";vinBtn.isEnabled=true}}catch(e:Exception){runOnUiThread{status.text="Connection failed: ${e.message}"}}}}
  vinBtn.setOnClickListener{status.text="VIN Scan en cours...";results.text="Lecture des informations du véhicule…"
   thread{try{val v=client.vehicleIdentity();runOnUiThread{info.text=v;card.visibility=View.VISIBLE;panel.visibility=View.VISIBLE;status.text="Véhicule identifié ✓ • Diagnostic disponible";results.text="Véhicule prêt. Lancez Test rapide, Codes défaut ou Données en direct."}}catch(e:Exception){runOnUiThread{results.text="VIN scan failed: ${e.message}\nKeep ignition ON and try again."}}}}
  findViewById<Button>(R.id.healthBtn).setOnClickListener{results.text="Contrôle de santé OBD…";thread{try{val r=client.liveData();runOnUiThread{results.text="SANTÉ OBD\n\n"+r}}catch(e:Exception){runOnUiThread{results.text="Erreur: ${e.message}"}}}}
  findViewById<Button>(R.id.infoBtn).setOnClickListener{thread{try{val r=client.vehicleIdentity();runOnUiThread{results.text="INFORMATIONS ECU / VÉHICULE\n\n"+r}}catch(e:Exception){runOnUiThread{results.text="Erreur: ${e.message}"}}}}
  findViewById<Button>(R.id.quickBtn).setOnClickListener{results.text="Test rapide en cours…\nInterrogation du calculateur moteur…";thread{try{val r=client.quickTest();runOnUiThread{results.text=r}}catch(e:Exception){runOnUiThread{results.text="Diagnostic error: ${e.message}"}}}}
  findViewById<Button>(R.id.scanBtn).setOnClickListener{results.text="Diagnostic en cours…\nLecture des codes défaut et données OBD…";thread{try{val r=client.fullScan();runOnUiThread{results.text=r}}catch(e:Exception){runOnUiThread{results.text="Diagnostic error: ${e.message}"}}}}
